	...
	*/	
	private function set_fields($page, $section) {
		foreach (NF_Settings::get_fields() as $id => $field) {
			$args = array('id'=> $id, 'type'=> $field['type']);
			$args['disable'] = isset($field['disable']) ? $field['disable'] : false;
			$field =  $args['disable'] ? '<div class="setting-disabled" />' . $field['title'] . '<div />' : $field['title'];
	...
	public function field_callback(array $arg) {
		...
		if ($arg['type'] == 'text') {
			...
			if ( isset($settings[$id])) {
				$checked =  $settings[$id] == 1 && ! $arg['disable'] ? ' checked': $checked;
			}
			echo '<input type="checkbox" '. $classes .' id="'. $id . '" name="' . $this->_table . '[' . $id . ']" value="1"' . $checked . $disabled . ' />';
